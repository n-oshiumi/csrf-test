...
try {
    ...
    $stmt = $pdo->prepare("SELECT * FROM posts INNER JOIN users ON users.id=posts.user_id");
    $res = $stmt->execute();
    ...
    if ($_SERVER ['REQUEST_METHOD'] === 'POST') {
        ...
        //データ追加
        $stmt = $pdo->prepare("INSERT INTO posts (content, user_id) VALUES(:content, :userId)");
...
<body>
    <header>
        ...
        <div class="container-fluid justify-content-center position-relative">
            <div class="navbar-header">
                <div class="navbar-brand text-white">CSRF脆弱性体験ができる掲示板</div>
            </div>
    ...
    <div class="body-wrapper p-5">
        <div class="notes mb-5">
            <p class="note text-gray mb-2">※当サイトはCSRF脆弱性を体験するためのサイトです。実際の掲示板としては運用していません。</p>
            <p class="note text-gray mb-2">※当サイトで得た知識を悪用しないでください。</p>
        </div>
        ...
        <form method="POST" action="/index.php">
            <div class="form-group ml-3">
                ...
                <button type="submit" class="btn btn-success mb-2">投稿する</button>
            </div>
        </form>
        <div class="card-wrapper mt-5">
            ...
            <div class="card mb-3">
                <div class="card-body">
                    <p><?= $post["email"]; ?> さんの投稿</p>
                    <p class="card-text"><?= $post["content"]; ?></p>
                    <a href="#" class="btn btn-success">見る</a>
                </div>
            </div>
            <?php endforeach; ?>
        </div>
